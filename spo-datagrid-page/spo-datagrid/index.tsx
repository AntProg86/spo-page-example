...
import { onExporting } from "#src/commonFunctions/fileFunctions";
...
import { setFormMode } from '../../reducer';
...
const SPODataGrid: React.FunctionComponent<Props> = () => {
  ...
  // Колбэк загрузки данных для CustomStore
	const loadData = (loadOptions:any) => {
		...
		return _appContext.doFetch(getSPOListFetch, {isExport: 1}, {signal})
			.then((data: any) => {
  ...
  //Загрузка данных (вызов load) и перерисовка датагрида
	...
	const onOpenClickHandler = (rowData: any) => {
		...
		//console.log(rowData.data.lineID);
		history.push({
			pathname:`/SPOFormPage/id=${rowData.data.spoid}`,
			state: {from: location},
		})
	};
  ...
  return (
    <>
      <DataGrid
				...
				onSelectionChanged={selectionChangeHandle}
				
        //Export в excel
        onExporting={(e) => onExporting(e, Ref_Datagrid.current?.instance, 'SPO Export')}
        ...
        <StateStoring
					enabled={true}
					type="custom"
					customLoad={useCallback(() => loadState(localStorageKeys.IE_TL_SPODataGrid), [])}
					customSave={useCallback((e: any) => saveState(e, localStorageKeys.IE_TL_SPODataGrid), [])}
					savingTimeout={5}
				/>
        ...
        <Toolbar>
          ...
          <Item location='after'>
					...
					<Item location='after'>
						<DxButton
							icon='add'
							hint={LocalizedStrings.add_SPO_export}
							onClick={addSPOExport}
						/>
					</Item>
          ...
          <Item location='after'>
						<DxButton
							...
							onClick={(e) => onExporting(e, Ref_Datagrid.current?.instance, 'SPO Export')}
						/>
					</Item>
          ...
          <Item location='after'>
						<TableSettingsDropDownButton
							localStorageKey={localStorageKeys.IE_TL_SPODataGrid}
							Ref_DataGrid={Ref_Datagrid}
							changePopupAction={changePopupStore}
							_appContext={_appContext} IsUseSaveLoadOptions={false}/>
					</Item>
